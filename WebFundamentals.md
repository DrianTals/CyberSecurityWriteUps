...
- **CNAME Record** - These records resolve to the address of the servers that handle the email for the domain you are querying, for example an MX record response for tryhackme.com would look something like alt1.aspmx.l.google.com. These records also come with a priority flag. This tells the client in which order to try the servers, this is perfect for if the main server goes down and email needs to be sent to a backup server.  
...
- The Root DNS Server directs it to the correct Top Level Domain (TLD) server (for example, the .com server).

- The TLD server tells it which Authoritative DNS Server (nameserver) is responsible for the domain.

- The Authoritative DNS Server sends back the correct IP address. The Recursive DNS Server saves it for a set time (TTL) and then sends it back to your computer.
